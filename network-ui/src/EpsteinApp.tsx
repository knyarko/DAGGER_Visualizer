...
interface EpsteinAppProps {
  onSwitchMode?: () => void;
}

function EpsteinApp({ onSwitchMode }: EpsteinAppProps = {}) {
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white">
      ...
      {/* Mode switch back to generic Data Explorer */}
      {onSwitchMode && (
        <button
          onClick={onSwitchMode}
          className="fixed top-2 right-2 z-50 text-[10px] text-gray-300 hover:text-white bg-gray-900/80 border border-gray-700 rounded px-2 py-1"
          title="Switch to generic Data Explorer"
        >
          ← Data Explorer
        </button>
      )}
